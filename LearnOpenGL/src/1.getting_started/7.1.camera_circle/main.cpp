...
int main()
{
    ...
    while (running)
    {
        ...
        ImGui::DragFloat3("Camera", cameraPos);
        float dt = stClk.getElapsedTime().asSeconds();
        static float radius = 10.f;
        ImGui::DragFloat("Radius", &radius);
        cameraPos[0] = cos(dt) * radius;
        cameraPos[2] = sin(dt) * radius;
        view = glm::lookAt(glm::vec3(cameraPos[0], cameraPos[1], cameraPos[2]), glm::vec3(0.f, 0.f, 0.f), glm::vec3(0.f, 1.f, 0.f));
        //
