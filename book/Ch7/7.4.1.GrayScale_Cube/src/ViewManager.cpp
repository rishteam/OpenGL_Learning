...
void ViewManager::keyEvents(unsigned char key) {
	...
	{
		m_camara.Translate(vec2(0, m_camara.moveSpeed));
	}
	else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Down))
	{
		m_camara.Translate(vec2(m_camara.moveSpeed, 0));
	}
	...
	case 'q':
		{
			...
			glm::vec3 rotateZAxis = (invrtRot * vec4(0, 0, 1, 0)).xyz;
			rotationMatrix = rotate(rotationMatrix, 0.02f, rotateZAxis);
		}
		break;
	case 'e':
		{
